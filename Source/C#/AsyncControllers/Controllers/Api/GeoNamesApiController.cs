...
namespace AsyncControllers.Controllers.Api
{
    public class GeoNamesApiController
    {
        [HttpGet]
        public IEnumerable<GeoName> GetLibraries()
        {
            return LoadLibraries();
        }

        private IEnumerable<GeoName> LoadLibraries()
        {
            ...
            var q = from m in geoNamesSet
                    where m.GeoNameId != dallasTx.GeoNameId                                             // We don't want the target in the list of results
                          ...
                          && (dallasTx.Latitude >= m.Latitude - LatitudeDeltatToMaxDistance
                              && dallasTx.Latitude <= m.Latitude + LatitudeDeltatToMaxDistance)         // Limit by latitude
                          && (dallasTx.Longitude >= m.Longitude - longitudeDeltaToMaxDistance
                              && dallasTx.Longitude <= m.Longitude + longitudeDeltaToMaxDistance)       // Limit by longitude
